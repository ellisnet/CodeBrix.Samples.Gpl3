...
**Read from a manuscript.** The **Manuscript Viewer** (Meta+Alt+A) puts PDF files *you* choose
beside the score — the engraver's copy you are transcribing, or notes, or a task list. Open
several at once and switch between them from a chooser; the same paged view gives you zoom, the
three fit modes, the magnifier and rubber-band **Copy to Image** over them; the panel's own
toolbar hides from its context menu; and the open manuscripts are saved with the named session and
come back with it. A PDF that carries point-and-click links behaves like the score: hover to
highlight the source, click to move the caret, Shift-click to edit in place. External hyperlinks
in a manuscript open in the desktop's own browser.
...
documents it. F1 opens the **user guide**: **70 pages** of the application's own documentation,
cross-linked, rendered natively (no embedded browser).

**Reach for it.** Under the menu bar sit the original's **two toolbars** — New, Open (with the
recent-files drop-down), Save, Close, Back, Forward, Undo, Redo, the Score Wizard and one
**Engrave** button that turns into a stop button while a job runs and engraves *custom* on
Shift-click — and beside them the Music View's own bar: the score chooser, zoom in/out with a
zoom-percentage chooser, the magnifier, page back and forward with a `3 of 12` page box, and
Clear. The icons are Frescobaldi's own light and dark sets, and the application draws whichever
one the desktop's colour scheme asks for.
...
The 25 programs under `Fresco.Brix/tools/` are deliberately **not** in the solution. They ship
nothing, they are built and run on demand, and three of them (`tools/manuals`,
`tools/symbolicons`, `tools/iconclean`) generate or copy in assets that are committed.
...
├─ THIRD-PARTY-NOTICES.txt           The attribution ledger: 11 sections, per-file licences
│
├─ src/
│  ├─ Fresco.Brix.UI/                Shared XAML (.shproj): App.xaml + Views/MainPage.xaml
│  ├─ Fresco.Brix.Core/              The application — 271 C# files, ~86,500 lines
...
│  │   ├─ Manuscripts/               The Manuscript Viewer's open-PDF list and link reader
...
│  │       ├─ icons/                 The toolbars' 19 icons in a light and a dark set, from
│  │       │                         Frescobaldi (Tabler, MIT + GPL-2.0-or-later), with the
│  │       │                         two licence/provenance notices that travel with them
...
│  ├─ libs/Fresco.Brix.MusicView/    qpageview subset — 22 files, ~5,900 lines: the paged
...
│
├─ tests/
│  ├─ Fresco.Brix.Core.Tests/            3,601 tests
│  ├─ libs/Fresco.Brix.Ly.Tests/           807 tests (+ the vendored MusicXML 4.0 schema)
│  └─ libs/Fresco.Brix.MusicView.Tests/    104 tests
│
└─ tools/                            25 repo programs that ship nothing:
   ├─ manuals, symbolicons, iconclean …generate or copy in committed assets (PDF manuals,
   │                                 symbol icons, the toolbars' two icon sets)
...
| `CodeBrix.Audio.MitLicenseForever` | 1.0.245.150 | SoundFont synthesis for MIDI playback and for rendering a score to WAV |
...
- **The largest single-application sample in the family, and it is still one codebase and six
  heads.** ~127,800 lines of C# across the core and two libraries, a dock shell with twelve
  dockable panels, twenty dialogs, a menu system, two toolbars, a shortcut system — and the six
  heads are still nothing but
...
- **Ports verified against the original, not against themselves.** Twenty-five programs under
  ...
  because upstream hangs on some inputs. The result is **4,512 tests** — 3,601 core, 807 language,
...
- **Internationalization done the hard, correct way.** The 13 shipped catalogs are Frescobaldi's
  ...
  reconciles 195 deliberately renamed strings — the ones this application had to reword because
