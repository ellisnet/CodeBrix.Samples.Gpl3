...
//     Break_alignment_interface::find_nonempty_break_align_group, landed with line
//     breaking -- this caller was written while it did not exist and nothing re-checked
//     it afterwards.
//   - minimum_distance carries upstream's Separation_item::conditional_skyline merge,
//     restored by the stale-stand-in class sweep; its "needs Separation_item" note had
//     been stale since that type landed. See PORT-COVERAGE.
...
public class PaperColumn : Item
{
    ...
    public static double MinimumDistance(Grob left, Grob right)
    {
        ...
        // The early omission of upstream's conditional-skyline merge retired with
        // the stale-stand-in class sweep: its callee had long since landed.
