...
/// <summary>
/// The volta and tuplet entry points: the grob callbacks for volta brackets, tuplet brackets and
...
/// </para>
/// </summary>
public static class VoltaAndTupletCallbacks
{
    ...
    // lily/stencil-scheme.cc, the four bindings this group's closing sweep DEMANDED,
    // pulled forward from the long-tail pool under the same rule as the spanner ones.
    //
    ...
    // lily/spanner-scheme.cc, PULLED FORWARD from the long-tail pool by this group's demand loop.
    //
    // The ledger files spanner-scheme.cc with the long-tail pool ("leaf binding file" —
    // the pool sweeps what a type's group did not land), but standing rule 3 says a *-scheme.cc file
    // is never a work item of its own: whoever ports a type owes its LY_DEFINE surface.
    // Spanner.cs landed long ago without this surface, and this is the group that first
    ...
    //
    // ly:spanner-broken-into is in the same file and is the binding the harness fix recorded
