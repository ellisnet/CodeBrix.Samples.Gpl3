...
/// <summary>
/// The Scheme entry points the bars/meter/keys/marks files declare: the breathing-sign
...
/// </para>
/// </summary>
public static class MeterAndKeyCallbacks
{
    ...
    /// <summary>
    /// Everything this group's landing DEMANDED from other owners, each block naming its
    /// owner: the pure-height and stencil-constructor leaf bindings the BarLine
    /// print/spacing path calls (the long-tail leaf pool over ported types),
    /// <c>ly:broadcast</c> (same pool), and zero-valued stand-ins for the alignment and
    /// break-alignment callbacks these grobs carry — without which the first multi-measure
    ...
    /// <c>euclidean-remainder</c>, MOVED to CodeBrix.LilyScheme's NumericPrimitives,
    /// which is where a Guile-core name belongs. They are gone from
    ...
    {
        // The two PURE height stand-ins that used to open this block are GONE.
        ...
        // with, so each returned an ORDINARY extent. The line-breaking group landed
        // unpure-pure-container.cc and the pure-height family, and both names are now
        // registered for real in LineBreakingCallbacks: ly:grob-pure-height through
        ...
        // of. They belong to stencil-scheme.cc (the long-tail leaf sweep; the Stencil type
        // and Lookup are ported), and this group's landing is what first demands them: a
        ...
        // Zero stand-ins for break-alignment-interface.cc anchors. A
        ...
        // implementations (BreakAlignInterface, registered by LineBreakingCallbacks,
        // which installs after this class) overwrite all three at install time.
        //
        // This block originally shipped the same zero stand-ins for nine
        // vertical-organization names (self-alignment x6, side-position x3); their REAL
        // implementations landed in the same wave, so those were removed at integration
        // and only the break-alignment trio remains.
        ...
        // ly:broadcast used to live here as a demand pull-forward. The whole of
        // dispatcher-scheme.cc is ported, so it now lives with its
