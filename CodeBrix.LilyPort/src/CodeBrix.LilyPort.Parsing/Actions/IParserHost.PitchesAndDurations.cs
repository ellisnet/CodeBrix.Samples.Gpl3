...
/// <content>
/// The members the PitchesAndDurations group needs: one more piece of <c>Lily_parser</c> state
/// (the remembered tremolo type, beside the EmbeddedScheme group's <c>DefaultDuration</c>) and the two
...
public partial interface IParserHost
{
    ...
    /// <c>scm/c++.scm</c>, so only the host can answer it, exactly like the PropertyPaths group's
