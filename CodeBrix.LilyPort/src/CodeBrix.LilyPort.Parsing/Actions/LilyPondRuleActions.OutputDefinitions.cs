...
/// <content>
/// Output definitions, paper and tempo: <c>paper_block</c>,
...
/// (ParserActionHelpers.OutputDefConstructors.cs), and they lean on Lily_lexer state more than any
...
{
    private static void RegisterOutputDefinitions(RuleActionTable table)
    {
        ...
        // (the TopLevel group's assignment action); the body only unwraps the marker list.
        table.Add(
            "output_def_body: output_def_body assignment",
            (context, values, locations, location)
        ...
        // StackValue/SetStackValue pair's business, as in the BookBlocks group's $@7.
        table.Add(
            "$@8: /* empty */",
            (context, values, locations, location) =>
            {
