...
declares the same order and ManualCatalogTests checks the two agree.
    ...
    learning         253 pages     2,801,037 bytes
    notation        1280 pages    12,490,418 bytes
    usage             96 pages       556,883 bytes
    extending         76 pages       444,259 bytes
    internals       1266 pages     7,306,510 bytes
    essay             63 pages     2,941,760 bytes
    music-glossary   135 pages       884,368 bytes
    changes           10 pages       102,985 bytes
    contributor      189 pages       958,888 bytes
    ----------------------------------------------
    TOTAL           3368 pages    28,487,108 bytes  (27.2 MB)

MEASURED 2026-08-26 at wave W11½ (the vector route), against LilyPort at
CodeBrix.LilyScheme 1.0.238.240 and Lily.Docs at CodeBrix.Texinfo 1.0.238.626
(Html2Pdf 1.0.238.580 on CodeBrix.Imaging.Drawing.NoSkia 1.0.238.425), and read
back with CodeBrix.PdfRasterizer 1.0.238.580 -- the same reader the application
uses. The page counts are read out of the SHIPPED files and they reproduce
Lily.Docs' own frozen baselines exactly; the nine files are the same size to
the byte as Lily.Docs' own render of the same day. The whole run took 9 min 04 s.

WHAT THE VECTOR ROUTE CHANGED, AND WHAT IT DID NOT. Every engraved example is
now placed as PDF vector content -- paths, with its lyrics and markup as real
embedded-font text -- where every render before this one placed a 192 ppi
bitmap. So: no page count moved (3,368 stands) and ManualCatalog is unchanged;
the files halved (54,171,714 -> 28,487,108 bytes; notation 33.8 -> 12.5 MB);
the engravings hold up at any zoom in the application's paged view; and their
text is selectable and searchable outside it (pdftotext finds the engine's
"LilyPond v2.27.2" tagline 2,025 times in notation and 32 times in essay). The
only bitmaps left are the manuals' real pictures: learning 5, notation 2,
essay 39 (+3 soft masks), contributor 1, none in the other five. Every render
reports "svg placement Vector"; every PDF passes `qpdf --check'.

//was previously: MEASURED 2026-08-21 at wave W10 against CodeBrix.LilyScheme
1.0.232.256 and CodeBrix.Texinfo 1.0.232.110 -- 54,171,714 bytes (51.7 MB) of
raster engravings -- and RE-VERIFIED unchanged the same day at the LilyScheme
1.0.233.918 / Texinfo 1.0.233.879 bump, with Lily.Docs' gates run ONCE ON EACH
SIDE of the Texinfo bump so that an engine change and a renderer change could
not be confused with one another (269/269 both sides, no regeneration).
