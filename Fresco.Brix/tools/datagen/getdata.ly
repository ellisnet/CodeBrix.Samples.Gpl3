%{
  Fresco.Brix repo tool (ships nothing): adapted from python-ly v0.9.10's
  ly/data/getdata.ly (GPL-3.0-or-later, Wilbert Berendsen) to run against the
  CodeBrix.LilyPort 2.27.2 engine — version bumped, sentinel lines added so the
  tool can cut the module out of a merged engraving log. The output is consumed
  by gen-lilypond-data.py, which writes Data/LilyPondData.g.cs.
%}
\version "2.27.2"
%{
