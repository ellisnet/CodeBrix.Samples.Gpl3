...
:   This mode colors items whose directions have been explicitly set with either
    ...
    or `\stemNeutral`, colors are reverted to black. Older engravers also
    reverted the highlighting of the other Layout Control mode; that was fixed
    long before the version {appname} engraves with, so it does not happen
    here.
