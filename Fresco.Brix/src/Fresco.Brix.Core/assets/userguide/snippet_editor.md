...
: The symbol to show in menu and snippet list. Symbols are icons that use the
  default text color and are shipped with the program.
...
!`LILYPOND_VERSION`
: The LilyPond release the built-in engraver is compatible with.

!`FRESCOBALDI_VERSION`
: The version of {appname}. The variable keeps its name so a snippet written
  against Frescobaldi goes on working.
