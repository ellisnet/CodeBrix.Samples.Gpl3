...
Using {menu_import}, you can import a MIDI file. The conversion is done by
`midi2ly`, which is built into {appname} and needs nothing installed.
...
What comes out of a MIDI file is a transcription rather than a score: MIDI
carries no beams, no slurs, no spelling of an accidental and no idea where a
voice begins, so the converter guesses the clef from the average pitch and
leaves the bar lines to the engraver.

Whatever the converter had to say while it worked is shown in {menu_log}.

#VARS
menu_import menu file -> submenu title|&Import/Export -> Import Midi...
