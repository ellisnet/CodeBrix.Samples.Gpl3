...
in the {appname} window (that's where the tool got its name from) but it can
...
Navigation and zooming provide the same functionality as the {music_view}
...
If the opened file is a PDF score engraved with point-and-click links
several function of {music_view} also work in the
manuscript viewer:
...
However, when that score should be engraved again the manuscript viewer
won't automatically update, but this can be done manually through the context
menu.

Note that the PDF files {appname} exports itself do not carry point-and-click
links: they are written from the engraved music directly, and the links are a
feature of the PDF a LilyPond run produces. A score PDF that does carry them
behaves in the manuscript viewer exactly as described above.
